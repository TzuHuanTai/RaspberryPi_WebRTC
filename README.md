...
1. build the native webrtc
2. build the signalr client with boringssl
3. create .net signalr server hub
4. try to use signalr-client to exchange ice/sdp info with webrtc.lib. check if conflict by libssl/libcrypto.

## Environment
* Ubuntu 20.04 64bit for WebRTC
* RaspberryPi OS 64bit for SignalR-Client* 
...
* Install some dependent packages
    ```bash
    sudo apt install python pulseaudio libpulse-dev build-essential libncurses5
    pulseaudio --start
    ```
